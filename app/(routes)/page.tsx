...
const HomePage = async () => {
  ...
  // === Product Slicing Logic ===
  // 1. Signature Dishes: First 6 items (Best for 3-column grid)
  const signatureDishes = products.slice(0, 6);
  
  // 2. Kitchen Specials: Next 8 items (Best for 4-column grid)
  const kitchenSpecials = products.slice(6, 14);

  // Custom vector line SVG for background decoration
  const CurveLine = () => (
  ...
  return (
    <Container className="pt-24 pb-10 px-4 sm:px-6 lg:px-8 space-y-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 w-full relative z-10 items-center">
          ...
          <div className="flex flex-col items-start gap-3">
            <h1 className="text-6xl md:text-8xl font-serif font-black text-primary leading-[0.9] tracking-tighter">
              Crafted to elevate your taste.
            </h1>
            <p className="text-lg md:text-xl text-primary/80 max-w-md font-medium leading-relaxed">
              Experience the creamy, tangy, and rich flavors of FlavorDesk. Rooted in culinary tradition, delivered fresh to your door.
            </p>
          ...
          <div className="relative h-[400px] md:h-[500px] w-full flex items-center justify-center">
            ...
            />
             ...
             <div className="absolute top-10 right-0 bg-[#F4EFE9] p-4 rounded-3xl shadow-lg border border-primary/10 max-w-[150px]">
                <p className="text-xs text-muted-foreground uppercase font-bold tracking-widest mb-1">Fat</p>
                <p className="text-xl font-serif text-primary">80.0 g</p>
             </div>

             {/* Floating Badge 2 */}
             <div className="absolute bottom-10 left-10 bg-primary text-[#F4EFE9] p-5 rounded-3xl shadow-lg">
                <p className="text-xs opacity-80 uppercase font-bold tracking-widest mb-1">Protein</p>
                <p className="text-2xl font-serif">2.5 g</p>
             </div>
          </div>
        </div>
      </section>

      {/* === 2. SEARCH & LOCATION (Floating Bar Style) === */}
      ...
      {/* === 4. FEATURED PRODUCTS (First 6 items) === */}
      <section>
        <div className="flex items-center justify-between mb-8 px-2">
      ...
      {/* === 7. KITCHEN SPECIALS (Next 8 items) === */}
      <section>
        <div className="flex items-center justify-between mb-8 px-2">
